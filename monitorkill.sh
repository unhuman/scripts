...
if [[ ${count} = 0 || ${processInfo} == '' ]]; then
   echo "Notice: No process ${processName} found." 
   exit 0
elif [[ ${multi} = false && ${count} -gt 1 ]]; then
   echo "Error: Multiple ${processName} (${count}) found."
   exit -2
fi 
...
echo "${processInfo}" | while IFS= read -r line ; do
   ...
   if [[ $cpu -gt maxCPU ]]; then
      echo "Notice: Killing ${processName} because CPU ${cpu} > ${maxCPU}"
      ...
      if [[ $k9Delay -gt 0 ]]; then 
         ...
         if [[ ${pid} != "" ]]; then
            echo "Notice: Kill -9 ${pid} ${processName} because it did not die"
            kill -9 ${pid}
         fi
      fi
   else
